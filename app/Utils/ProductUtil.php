...
class ProductUtil extends Util
{
     ...
     * @return boolean
     */
    public function decreaseProductQuantity($product_id, $variation_id, $location_id, $new_quantity, $old_quantity = 0,$kurang_tambah = null)
    {
        ...
        if ($product->enable_stock == 1) {
            ...
            $var = VariationLocationDetails::where('variation_id', $variation_id)
                ->where('product_id', $product_id)
                ->where('location_id', $location_id);

            if($kurang_tambah == 'kurang' || $kurang_tambah == null){
              $var->decrement('qty_available', $qty_difference);
            }else{
              $var->increment('qty_available', $qty_difference);
            }
